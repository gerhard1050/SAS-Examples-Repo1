                        ...
                        *** ====== RAILROADS ======  ***;
                        if PlayerPos[Player] in (6,16,26,36) then do;
                                if Field[PlayerPos[Player]] = 0 then do;
                                    ...
                                    RailRoad_Count = 0;
                                    do i = 1 to 4;
                                        if Field[scan("6 16 26 36", i)] = Field[PlayerPos[Player]] then RailRoad_Count + 1;
                                    end;

                                    select (RailRoad_Count);
                                        when (1) Rent = 25;
                                        when (2) Rent = 50;
                                        when (3) Rent = 100;
                                        when (4) Rent = 200;
...
proc rank data=work.Player_LocationWallet
          out = work.player_ranking(drop = value feature)
          ties=low descending;
by game round;
...
data work.Player_Stats;
length Game Round 8 Feature $8 Player Value 8;
set work.Player_LocationWallet
    work.player_Ranking (in=in2)
;
if in2 then do;
   Feature = "Ranking";
   Value = Ranking;
...
proc sort data=work.Player_Stats;
...
/**************************************************************
*** Append Player + Fields
***************************************************************/
data work.ResultsRepo;
 set work.Player_Stats
     work.Field_Stats;
run;
